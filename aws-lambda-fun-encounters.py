...
# define a function authenticate and create access token for access player encounters in world of warcarft game
...
def create_access_token(client_id, client_secret, region, realmSlug, characterName):
    ...
    # use access token, realmSlug and characterName for fetching all encounters of the player
    
    result = requests.get('https://{reg}.api.blizzard.com/profile/wow/character/{slug}/{name}/encounters?namespace=profile-us&locale=en_US&access_token={access_token}'.format(
        reg=region, slug=realmSlug, name=characterName, access_token=response['access_token']))
    
    return result.json()

# open a gameclient table for insert result of encounters of the player in gameclient table
