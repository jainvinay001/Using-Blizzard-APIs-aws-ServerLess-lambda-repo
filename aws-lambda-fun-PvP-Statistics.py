...
# define a function authenticate and create access token for access player pvp statistics in world of warcarft game
...
def create_access_token(client_id, client_secret, region, realmSlug, characterName):
    ...
    # use access token, realmSlug and characterName for fetching all pvp-bracket/3v3 of the player
    
    result = requests.get('https://{reg}.api.blizzard.com/profile/wow/character/{slug}/{name}/pvp-bracket/3v3?namespace=profile-us&locale=en_US&access_token={access_token}'.format(
        reg=region, slug=realmSlug, name=characterName, access_token=response['access_token']))
    
    return result.json()

# open a gameclient table for insert result of pvp statistics of the player in gameclient table
